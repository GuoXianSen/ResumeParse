...
from apps.platformsettings.views import LunbotuManageViewSet, OtherManageViewSet, PlatformImagesUploadView, \
    UserLeavingMessageViewSet, SystemConfigViewSet, PlatformFileUploadView
...
urlpatterns = [
    ...
    path('uploadplatformfile/<str:path>/', PlatformFileUploadView.as_view(), name='后台上传平台设置上传文件'),
    path('uploadplatformfile/', PlatformFileUploadView.as_view(), name='后台上传平台设置上传文件'),
