...
from utils.imageuploadinstant import ImageUploadInstant
...
class PlatformImagesUploadView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        # 文件上传之后返回的处理结果
        result = ImageUploadInstant(request, "platform")
        if result['code'] == 200:
            return SuccessResponse(data=result['zzz'], msg=result['msg'])
        else:
...
class PlatformFileUploadView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        result = FileUpload(request, "platform")
        if result['code'] == 200:
            return SuccessResponse(data=result['zzz'], msg=result['msg'])
        else:
            return ErrorResponse(msg=result['msg'])
...
class APPUserLeavingMessageView(APIView):
    ...
    def post(self, request):
        ...
        UserLeavingMessage.objects.create(user=request.user, message=message, subject=message, images=images,
                                          message_type=1)
