...
class SmsSerializer(serializers.Serializer):
    ...
    def validate_mobile(self, mobile):
        ...
        if self.context['smstype'] == "restpass" or self.context['smstype'] == "login":  # 重置密码/用户登录
            # 该手机号是否已注册
            if not Users.objects.filter(username=mobile + "app", mobile=mobile, identity__contains="1",
                                        is_active=True).count():
                raise serializers.ValidationError("没有找到该用户", 400)

        if self.context['smstype'] == "wxbind":  # 微信绑定
            # 该手机号是否已注册
            if not Users.objects.filter(username=mobile + "app", mobile=mobile, identity__contains="1", is_active=True,
