...
from apps.mall.models import GoodsCategory, GoodsCoupon, CouponRecord, SPU, SKU, SKUImage, SKUSpecification, \
    SPUSpecification, SPUSpecificationOption, OrderInfo, OrderGoods, CouponRecord, GoodsCoupon, OrderRefunds
from apps.address.models import Address
from utils.jsonResponse import SuccessResponse, ErrorResponse
from rest_framework import serializers
from utils.common import formatdatetime, float2dot, REGEX_MOBILE, get_parameter_dic, renameuploadimg, \
    getminrandomodernum, geturlpath, ismoney, ast_convert, getrandomodernum, format_wechat_gmt_8_to_normal
import re
...
class GoodsSPUSerializer(CustomModelSerializer):
    ...
    def create(self, validated_data):
        ...
        spu_sepcs = validated_data.pop(
            'spu_specs')  # 商品的spu规格字典数组形式[ { "name": "颜色", "options": [ { "value": "白色" }, { "value": "黑色" } ] }, { "name": "大小", "options": [ { "value": "X" }, { "value": "S" } ] } ]
                ...
                if spec_type:  # 多规格
                    ...
                    for spu_sepc in spu_sepcs:
                        ...
                        for spu_sepc_option in spu_sepc['options']:
                            new_spu_spec_option = SPUSpecificationOption.objects.create(spec_id=new_spu_spec.id,
                                                                                        value=spu_sepc_option['value'])
                    for sk in skus:
                        if 'default_image' not in sk.keys():
                            sk['default_image'] = ""
                        sku_instance = SKU.objects.create(spu_id=spu.id, price=sk['price'], stock=sk['stock'],
                                                          default_image=sk['default_image'])
                        for sp in sk['specs']:
                            spuspec_instance = SPUSpecification.objects.filter(spu_id=spu.id, name=sp['spec']).first()
                            spuspec_option_instance = SPUSpecificationOption.objects.filter(spec_id=spuspec_instance.id,
                                                                                            value=sp['option']).first()
                            SKUSpecification.objects.create(sku_id=sku_instance.id, spec_id=spuspec_instance.id,
                                                            option_id=spuspec_option_instance.id)
                    ...
                    if len(skus) <= 0:
                        ...
                        return
                    SKU.objects.create(spu=spu, price=skus[0]['price'], stock=skus[0]['stock'],
                                       default_image=skus[0]['default_image'])
    ...
    class Meta:
        model = SPU
        # fields = "__all__"
        exclude = ['dept_belong_id', 'description', 'modifier', 'creator', 'unit', 'brand', 'comments', 'desc_pack',
                   'desc_service']
        read_only_fields = ["id"]
        extra_kwargs = {
        ...
        with transaction.atomic():  # 开启事务
            try:
                ...
                if spec_type:  # 多规格（只能修改现有规格库存和价格，不能添加和减少规格）前端直接修改，不通过本接口修改
                    ...
                    SKU.objects.filter(spu_id=instance.id, id=skus[0]['id']).update(price=skus[0]['price'],
                                                                                    stock=skus[0]['stock'],
                                                                                    default_image=skus[0][
                                                                                        'default_image'])
            except Exception as e:
                # 回滚到保存点
                transaction.savepoint_rollback(savepoint)
...
class OrderInfoSerializer(CustomModelSerializer):
    ...
    def get_address(self, obj):
        data = {}
        data['areas'] = obj.address.get_region_format() + (
            obj.address.street if obj.address.street else "") + obj.address.place
...
def orderrefund(orderid, type=1):
    ...
    if paymethod == 2:  # 微信
        wxapppay = WxAppPay()
        data = wxapppay.refundsorder(orderno, transaction_id, reason, money * 100, money * 100,
                                     notify_url="%s/api/app/wechatpay_notify_refund/" % DOMAIN_HOST)
    elif paymethod == 3:  # 支付宝
        data = alipay_trade_refund(transaction_id, float(money),
                                   notify_url="%s/api/app/ali_notify_refund/" % DOMAIN_HOST)
...
class GoodsListView(APIView):
    ...
    def get(self, request):
        ...
        if search:
            if is_tuijian == 2:
                if category:
                    queryset = SPU.objects.filter(name__contains=search, category1_id=category,
                                                  is_launched=True).order_by(order_by)
                else:
                    queryset = SPU.objects.filter(name__contains=search, is_launched=True).order_by(order_by)
            else:
                queryset = SPU.objects.filter(name__contains=search, is_tuijian=is_tuijian, is_launched=True).order_by(
                    order_by)
        else:
            if is_tuijian == 2:
                if category:
        ...
        for temp_sku in skus:
            ...
            if spu.spec_type == 1:  # 多规格
                for temp in sku_spec_options:
                    sku_options['spec'].append(
                        {'spec_key': temp.spec.name, 'spec_value': temp.option.value, 'spec_key_id': temp.spec_id,
                         'spec_value_id': temp.option_id})
...
class MyCouponView(APIView):
    ...
    def get(self, request):
        ...
        if type:
            if type == 10:
                if status == 10:
                    queryset = CouponRecord.objects.filter(user=user, status__in=[0, 1], coupon__coupon_type__in=[0, 1],
                                                           is_delete=False).order_by('-create_datetime')
                else:
                    queryset = CouponRecord.objects.filter(user=user, status=status, coupon__coupon_type__in=[0, 1],
                                                           is_delete=False).order_by('-create_datetime')
            elif type == 20:
                if status == 10:
                    queryset = CouponRecord.objects.filter(user=user, status__in=[0, 1], coupon__coupon_type__in=[0, 2],
                                                           is_delete=False).order_by('-create_datetime')
                else:
                    queryset = CouponRecord.objects.filter(user=user, status=status, coupon__coupon_type__in=[0, 2],
                                                           is_delete=False).order_by('-create_datetime')
            else:
                if status == 10:
                    queryset = CouponRecord.objects.filter(user=user, status__in=[0, 1], coupon__coupon_type=type,
                                                           is_delete=False).order_by('-create_datetime')
                else:
                    queryset = CouponRecord.objects.filter(user=user, status=status, coupon__coupon_type=type,
                                                           is_delete=False).order_by('-create_datetime')
        else:
            if status == 10:
                queryset = CouponRecord.objects.filter(user=user, status__in=[0, 1], is_delete=False).order_by(
                    '-create_datetime')
            else:
                queryset = CouponRecord.objects.filter(user=user, status=status, is_delete=False).order_by(
                    '-create_datetime')
        data = []
        if queryset:
            for m in queryset:
...
class OrdersCommitView(APIView):
    ...
    def post(self, request):
        ...
        coupon = ""
        if coupon_id:
            coupon = CouponRecord.objects.filter(id=coupon_id, status=1, coupon__coupon_type__in=[0, 1], user=user,
        ...
        if type == 1:  # 购物车订单提交
            ...
            with transaction.atomic():
                try:
                    ...
                    for sku_id in sku_ids:
                        ...
                        for i in range(5):
                            ...
                            res = SKU.objects.filter(id=sku_id, stock=stock_old, sales=sales_old).update(
                                stock=stock_new, sales=sales_new)
                            if res == 0:
                ...
                return SuccessResponse(
                    data={'id': order.id, 'order_id': order.order_id, 'total_amount': order.total_amount},
                    msg='success')
        else:  # 直接购买
        ...
        with transaction.atomic():
            ...
            for s in skus:
                ...
                for i in range(5):
                    ...
                    # 乐观锁step3: 基于旧库存和销量过滤查找模型类对象，然后更新
                    # 查询集批量修改函数update有整数返回值 —— 表示被修改了的对象有几个再次按照旧的库存获取商品,获取到更新,获取不到返回再次操作(乐观锁)
                    res = SKU.objects.filter(id=s.sku.id, stock=stock_old, sales=sales_old).update(stock=stock_new,
                                                                                                   sales=sales_new)
                    if res == 0:
                        # (3.2)、说明没有数据被成功修改，继而说明filter过滤出空查询集，进一步说明"根据旧库存找不到原有的sku，有别的事务介入"
                        continue
...
class GoodsOrdersListView(APIView):
    ...
    def get(self, request):
        ...
        if search:
            if status:
                status = int(status)
                queryset = OrderInfo.objects.filter(order_id__contains=search, user=user, is_delete=False,
                                                    status=status).order_by('-create_datetime')
            else:
                queryset = OrderInfo.objects.filter(order_id__contains=search, user=user, is_delete=False).order_by(
                    '-create_datetime')
        else:
            if status:
                status = int(status)
                queryset = OrderInfo.objects.filter(user=user, is_delete=False, status=status).order_by(
                    '-create_datetime')
            else:
            ...
            try:
                ...
                if paymethod == 2:
                    ...
                    if platform == 'app':  # app支付
                        data = wxapppay.payorder(orderno, money * 100, orderno, paytype,
                                                 notify_url="%s/api/app/wechatpay_notify/" % DOMAIN_HOST)  # 要填写回调地址
                    else:
                        openid = OAuthWXUser.objects.filter(user=user).values_list('xcx_openid', flat=True).first()
                        if not openid:
                            # 回滚到保存点
                            transaction.savepoint_rollback(save_point)
                        data = wxapppay.payorder_jsapi(orderno, money * 100, orderno, paytype, openid,
                                                       notify_url="%s/api/app/wechatpay_notify/" % DOMAIN_HOST)  # 要填写回调地址
                    # 事物结束提交数据
                    transaction.savepoint_commit(save_point)
                    return SuccessResponse(
                        data={'id': order.id, 'order_no': order.order_id, 'paymethod': 2, 'paystatus': order.pay_status,
                              'wechataydata': data}, msg="success")
                elif paymethod == 3:
