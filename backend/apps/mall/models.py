...
class GoodsCategory(CoreModel):
    ...
    parent = models.ForeignKey('self', related_name='goodscategorysubs', null=True, blank=True,
...
class GoodsCoupon(CoreModel):
    ...
    use_min_price = models.DecimalField(max_digits=10, decimal_places=2, verbose_name='最低消费多少金额可用优惠券',
                                        default=0)  # 有无门槛,0表示无门槛
    ...
    coupon_expiretime = models.IntegerField(default=0, verbose_name="优惠券有效期限（单位：天）",
                                            help_text="优惠券有效期限（单位：天）")  # 领取后几天有效，自券被领时开始算起
    receive_type = models.SmallIntegerField(choices=GOODSCOUPON_RECEIVE_TYPE, default=0, verbose_name="优惠券发送方式",
                                            help_text="优惠券发送方式")
    coupon_type = models.SmallIntegerField(choices=GOODSCOUPON_COUPONTYPE, default=0, verbose_name="优惠券类型（2）",
                                           help_text="优惠券类型（2）")
    is_delete = models.BooleanField(default=False, verbose_name="是否逻辑删除", help_text="是否逻辑删除")
...
class GoodsBrand(CoreModel):
    """ 商品所属品牌 """
    category = models.ForeignKey(GoodsCategory, on_delete=models.CASCADE, related_name="brands", verbose_name="所属分类",
                                 help_text="所属分类")
...
class SPU(CoreModel):
    ...
    brand = models.ForeignKey(GoodsBrand, on_delete=models.PROTECT, related_name="brands", blank=True, null=True,
                              verbose_name="品牌")
...
class SKU(CoreModel):
    ...
    shop_price = models.DecimalField(max_digits=10, decimal_places=2, default=0, null=True, blank=True,
                                     verbose_name="售价")
    market_price = models.DecimalField(max_digits=10, decimal_places=2, default=0, null=True, blank=True,
                                       verbose_name="市场价")
...
class OrderInfo(CoreModel):
    ...
    couponrecord_price = models.DecimalField(max_digits=10, decimal_places=2, default=0, null=True, blank=True,
                                             verbose_name="优惠券面值")
