...
class UsersCVManageTimeFilter(django_filters.rest_framework.FilterSet):
    """
    简历上传解析数据管理 简单过滤器
    URL格式：http://127.0.0.1:8000/?start_time=2020-12-02 12:00:00&end_time=2021-12-13 12:00:00
    field_name: 过滤字段名，一般应该对应模型中字段名
    ...
    # 模糊搜索
    school = django_filters.CharFilter(field_name='school', lookup_expr='icontains')  # icontains表示该字段模糊搜索
    # 模糊搜索
    xueli = django_filters.CharFilter(field_name='xueli', lookup_expr='icontains')  # icontains表示该字段模糊搜索
    # 模糊搜索
    name = django_filters.CharFilter(field_name='name', lookup_expr='icontains')  # icontains表示该字段模糊搜索
    # 模糊搜索
    nianxian = django_filters.CharFilter(field_name='nianxian', lookup_expr='icontains')  # icontains表示该字段模糊搜索
    # 模糊搜索
    mobile = django_filters.CharFilter(field_name='mobile', lookup_expr='icontains')  # icontains表示该字段模糊搜索
    is_active = django_filters.CharFilter(field_name='is_active')  # icontains表示该字段模糊搜索
    ...
    class Meta:
        model = CvUploadinfo
        fields = ['name', 'xueli', 'is_active', 'nianxian', 'school']
