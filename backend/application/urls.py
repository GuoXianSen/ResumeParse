...
from apps.oauth.views import WeChatXCXLoginAPIView, XCXWeChatUserInfoUpdateAPIView, WeChatXCXMobileLoginAPIView, \
    WeChatGZHLoginAPIView, WeChatGZHBindAPIView, GetXCXShareQrcodeView, TTXCXLoginAPIView, WeChatGZHH5LoginAPIView, \
    CheckWeChatGZHH5APIView, GetWeChatGZHH5JSSDKTempSignAPIView
from apps.address.views import *
from apps.logins.views import UsernamePassWordLoginView, APPMobilePasswordLoginView, SendSmsCodeView, \
    APPMobileSMSLoginView, ForgetPasswdResetView, RegisterView
...
urlpatterns = [
    ...
    path('gyzapi/', schema_view.with_ui('swagger', cache_timeout=0), name='schema-swagger-ui'),
    ...
    # 管理后台其他自定义接口
    # 简历测试接口
    path('api/cv/', include('apps.cvtest.urls')),
    path('api/cvpost/', include('apps.cvpost.urls')),
    path('api/cvupload/', include('apps.cvupload.urls')),
